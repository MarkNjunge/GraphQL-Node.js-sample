...
const resolvers = {
  ...
  Mutation: {
    register(_, args) {
      return User.register(args.fullname, args.email, args.photoUrl, args.password)
        .catch(reason => {
    ...
    updateUser: baseResolver.createResolver(
      (_, args, context) => {
        return User.update(context.user.id, args.fullname, args.email, args.photoUrl)
          .catch(reason => {
            throw new CustomErrors.BasicError({
              message: reason
            })
    ...
    changePassword: baseResolver.createResolver(
      (_, args, context) => {
        return User.changePassword(context.user.id, args.currentPassword, args.newPassword)
          .catch(reason => {
            throw new CustomErrors.BasicError({
              message: reason
            })
